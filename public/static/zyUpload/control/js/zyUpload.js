	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '<form id="uploadForm1" action="'+para.url+'" method="post" enctype="multipart/form-data">';
					...
					// 创建不带有拖动的html
					html += '<form id="uploadForm2" action="'+para.url+'" method="post" enctype="multipart/form-data">';
					html += '	<div class="upload_box">';
				    ...
				    html += '							<img class="upload_image" src="/static/zyUpload/control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.createCorePlug = function(){
				...
				var params = {
					...
					onSuccess: function(file, response) {
                        avater.push(response);
						$('#avater').val(avater);
						$("#uploadProgress_" + file.index).hide();
						$("#uploadSuccess_" + file.index).show();
						//$("#uploadInf").append("<p>上传成功，文件地址是：" + response + "</p>");
						layer.msg("上传成功，文件地址是:"+avater);
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
